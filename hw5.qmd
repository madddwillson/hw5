...
lm_fit <- lm(median_house_value ~ latitude + longitude + housing_median_age + total_rooms + 
                total_bedrooms + population + median_income + ocean_proximity, data = df_train)
summary(lm_fit)
```
...
```{R}
rpart_predictions <- predict(rpart_fit, newdata = df_test)
rmse_value <- sqrt(mean((df_test$median_house_value - rpart_predictions)^2))
```
...
NNet <- nn_module(
    ...
    self$hidden1 <- linear(p, q1)
    self$hidden2 <- linear(q1, q2)
    self$hidden3 <- linear(q2, q3)
    self$output <- linear(q3, 1)
    },
    forward = function(x){
      x %>% 
        self$hidden1() %>% relu() %>% 
...
nnet_fit <- NNet %>% 
  setup(
    df_train[, -ncol(df_train)], 
    df_train$median_house_value
  ) %>%
  set_hparams(32, 16, 8) %>%
  set_opt_params(loss_fn = nn_mse_loss(),
...
```R
# Plot loss
plot(train_loss, type = "l", col = "blue", xlab = "Epoch", ylab = "Loss", main = "Training and Validation Loss")
lines(valid_loss, col = "red")
legend("topright", legend = c("Training Loss", "Validation Loss"), col = c("blue", "red"), lty = 1)

# If available, plot accuracy
if (exists("train_accuracy")) {
  plot(train_accuracy, type = "l", col = "blue", xlab = "Epoch", ylab = "Accuracy", main = "Training and Validation Accuracy")
  lines(valid_accuracy, col = "red")
...
results_table <- tibble(
  ...
  RMSE = c(lm_rmse, rpart_rmse, svm_rmse, rmse(df_test$median_house_value, as_array(nnet_predictions)))
)
...
rpart_classes <- predict(rpart_fit, newdata = df_test, method = "class")
```
...
```{R}
# Compute accuracy
rpart_classes <- mean(rpart_classes == df_test$spam)
rpart_classes
```
...
nnet_fit <- NNet %>% 
  setup(
    ...
    hidden_layers = list(
      DenseLayer(units = 32, activation = "relu"),
...
results <- data.frame(
  ...
  Accuracy = c(accuracy_glm, accuracy_rpart, accuracy_svm, accuracy_nn)
)
...
```R
rpart_fit <- rpart(y ~ ., data = df, method = "class")
